...
Experiment name:                20200507 05_15_ CI and PEIS for bubble test

                                FOR SINGLE CI ANALYSIS
"""
...
if plot_RvsI:
    ...
    for filename, CI_data in CI1.data.items():
        ...
        R = CI_data['Ru/Ohm'].iloc[-1]
        j = CI_data['control/mA'].iloc[1]/CI1.A_electrode
    ...
    pltF.CA_global(axs[0], ylabel = CI_data['Ru/Ohm'].name, xlabel = None, legend = False)
    pltF.CA_global(axs[1], ylabel = CI_data['Ru/Ohm'].name, xlabel = 'I/mAcm-2', legend = False)
    # ax.set_xlim(left = 0)      
    # pltF.global_legendbox(ax[1])
    # axs[1].set_ylim(bottom = 0)
